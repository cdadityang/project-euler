...
Ans: We'll take it as string. Then grid array will be of 3D -> First we split lines and get a 2D array have 20 elements with each element having one line. Then we again map each element of array in to array i.e., making it 3D and convert string to integer and store in grid. The logic is simple then, we iterate in directions from 0 to 16(19-3) by taking product of 4 adjacent integers and then finding the highest of all and storing it in "biggestproduct" variable.
...
grid = numbers_in_string.lines.map{|line| line.split(' ').map{ |j| j.to_i }}
biggestproduct = 0
factors = []
direction = nil
#Horizontal
